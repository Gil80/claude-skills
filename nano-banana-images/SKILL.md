...
After constructing the JSON prompt, you **MUST** ask the user which backend to use before executing. Present these options:

1. **Kie.ai API** — Highest quality, requires `KIE_API_KEY` in `.env`
2. **Google Gemini API** — Direct Gemini call, requires `GEMINI_API_KEY` in `.env`
3. **Free cloud SDXL** — No API key needed, good quality (Puter.ai)
4. **Local SDXL** — No API key, runs on local GPU/CPU (slow, downloads ~5GB model)
5. **Export for Gemini web UI** — Outputs plain text prompt to copy/paste into Gemini's "Create Images" tool

### Backend Commands

All commands run from the project root: `/home/gil/projects/nano-banana-images/`
...
1. Construct the prompt using the JSON schema above
2. Save it to a `.json` file in the `prompts/` directory
3. Ask the user which backend to use (present the 5 options above)
4. Execute the selected backend command
